...
@click.pass_context
def events(ctx, metric_id, limit, profile_id):
    ...
    unsuppressed). Metric IDs come from list-metrics.
    """
    use_json = ctx.obj["json"]
    try:
        filter_str = f'equals(metric_id,"{metric_id}")'
        if profile_id:
            filter_str = f'and({filter_str},equals(profile_id,"{profile_id}"))'
        encoded = urllib.parse.quote(filter_str)
        path = (f"/api/events/?filter={encoded}&sort=-datetime"
                f"&include=profile&page[size]={limit}")
        data = ctx.obj["call"]("GET", path)

        if use_json:
            output(data, use_json=True)
            return
        included = {i["id"]: i.get("attributes", {})
                    for i in data.get("included", []) if i.get("type") == "profile"}
        items = data.get("data", [])
